...
case "$OS" in
    ubuntu|debian)
        ...
        apt-get update -qq
        apt-get install -y -qq python3 python3-pip python3-venv nodejs npm git nginx curl ufw > /dev/null 2>&1
        ;;
    centos|rocky|almalinux|rhel)
        dnf install -y python3 python3-pip nodejs npm git nginx curl firewalld > /dev/null 2>&1
        ;;
    *)
